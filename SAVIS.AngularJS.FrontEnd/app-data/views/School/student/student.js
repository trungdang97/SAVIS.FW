define(['jquery', 'app', 'angular-sanitize',
    ...
    'views/School/student/student-item',
    'components/formly-template/formly-factory',
], function (jQuery, app) {
    app.controller('StudentCtrl', ['$scope', '$sce', '$timeout', '$location', '$log', '$http', '$uibModal', 'constantsFactory',
        'constantsAMD', '$routeParams', 'Notifications', 'StudentService', 'FormlyFactory',
        function ($scope, $sce, $timeout, $location, $log, $http, $uibModal, constantsFactory,
            ...
            var itemDialogUrl = '/app-data/views/School/student/student-item.html';
            $scope.ListSelected = [];
            $scope.SelectAll = false;

            /* Header grid datatable */
            $scope.Headers = [
                { Key: '#', Value: "Mã sinh viên", Width: '15%', Align: 'text-center' },
                { Key: '#', Value: "Tên sinh viên", Width: '25%', Align: 'text-center' },
                { Key: '#', Value: "Ngày sinh", Width: '15%', Align: 'text-center' },
                { Key: '#', Value: "Lớp học", Width: '10%', Align: 'text-center' },
                { Key: '#', Value: "Chức vụ trong lớp", Width: 'auto', Align: 'text-center' },
                ...
                var loadData = function () {
                    ...
                    var promise = StudentService.GetFilter(postData);
                    $scope.count_req = 0;
            ...
            $scope.Button.Create.Click = function () {
                ...
                modalInstance = $uibModal.open({
                    ...
                    controller: 'StudentItemCtrl',
                    size: 'lg',
                    // windowStudent :"modal-full",
                    backdrop: 'static',
            ...
            $scope.Button.DeleteMany.Click = function () {
                ...
                angular.forEach($scope.ListData, function (item) {
                    if (item.Selecting) {
                        listDeleteting.push(item.StudentId);
                    }
                });
